...
Deno.serve(async (req) => {
  ...
  if (action === "start") {
    ...
    if (players.length > maxPlayers) return json({ error: `This room is capped at ${maxPlayers} players.` }, 400);
  ...
  if (action === "play" || action === "draw") {
    ...
    if (action === "draw") {
      ...
      if (myHand.length === 0) {
        ...
        // Record this win in room_stats — read-modify-write is safe
        // here because this function is the only writer of this row.
        const { data: winnerRow } = await admin.from("players").select("name").eq("id", callerId).maybeSingle();
        const { data: stats } = await admin.from("room_stats").select("*").eq("room_id", roomId).maybeSingle();
        const wins = { ...(stats?.wins || {}) };
        wins[callerId] = { name: winnerRow?.name || "Player", wins: (wins[callerId]?.wins || 0) + 1 };
        await admin.from("room_stats").upsert({
          room_id: roomId, games_played: (stats?.games_played || 0) + 1, wins, updated_at: new Date().toISOString(),
        });
